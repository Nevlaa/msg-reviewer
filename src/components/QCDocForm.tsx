            ...
            <div className="inventory-table-container">
                  ...
                  {data?.results?.food_inventory?.map((item: any, idx: number) => {
                    ...
                    return (
                      <tr key={idx} style={{ 
                        ...
                        background: isItemFound ? (rowPass ? '#f0fdf4' : '#fefce8') : '#fef2f2'
                      }}>
                        ...
                        <td style={{ padding: '5px 6px', color: isItemFound ? (isCountClose ? '#059669' : '#b45309') : '#dc2626', fontStyle: isItemFound ? 'normal' : 'italic' }}>
                          {isItemFound ? item.ai_match_name : 'Not found in images'}
                        </td>
              ...
              {data?.results?.food_inventory?.length > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 8px', background: '#f1f5f9', fontSize: '0.65rem', color: '#475569', borderTop: '2px solid #cbd5e1' }}>
                  <span>
                    <strong>Verified:</strong> {data.results.food_inventory.filter((i: any) => !!i.ai_match_name).length} / {data.results.food_inventory.length} items
                  </span>
                  <span>
                    <strong>3x3 Pass:</strong> {data.results.food_inventory.filter((i: any) => !!i.ai_match_name && (parseInt(i.actual_found) >= 3 || String(i.actual_found || '').includes('+'))).length} items
                  </span>
